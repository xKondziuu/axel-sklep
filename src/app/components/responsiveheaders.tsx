...
export function H2(props: PropsWithChildren<HeaderProps>) {
  return (
    <div className={`${!props.fo && 'font-opensans'} font-bold tracking-wide opacity-90`}>
      <h2 className="text-3xl xl:text-[1.75rem] lg:text-[1.6rem] xs:text-[1.15rem]">{props.children as string}</h2>
    </div>
  )
}

export function H3(props: PropsWithChildren<HeaderProps>) {
  return (
    <div className={`${!props.fo && 'font-opensans'} font-bold tracking-wide opacity-90`}>
...
export function H4(props: PropsWithChildren<HeaderProps>) {
  return (
    <div className={`${!props.fo && 'font-opensans'} font-bold tracking-wide opacity-90`}>
...
export function H5(props: PropsWithChildren<HeaderProps>) {
  return (
    <div className={`${!props.fo && 'font-opensans'} font-semibold tracking-wide opacity-90`}>
      <h5 className="text-xl xl:text-[1.1rem] lg:text-[1.05rem] xs:text-[0.95rem]">{props.children as string}</h5>
    </div>
  )
}

export function H6(props: PropsWithChildren<HeaderProps>) {
  return (
    <div className={`${!props.fo && 'font-opensans'} font-semibold tracking-wide opacity-90`}>
